...
class TestZappyClient(unittest.TestCase):
    def setUp(self):
        ...
        self.client = ZappyClient(self.hostname, self.port, self.team_name)
    ...
    def test_connect(self, mock_socket):
        ...
        mock_socket_instance.connect.assert_called_once_with((self.hostname, self.port))
        mock_socket_instance.send.assert_called_with(self.team_name.encode())
