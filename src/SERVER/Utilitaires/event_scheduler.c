...
static const command_t *find_soonest_command_in_queue(command_queue_t *queue)
{
    ...
    for (int i = 1; i < queue->size; ++i) {
        ...
        if (timespec_cmp(&queue->commands[index].ready_at,
            &soonest_cmd->ready_at) < 0) {
...
static long calculate_next_event_cmd(command_queue_t *queue,
    struct timespec* now, long min_t)
{
...
int calculate_next_event_timeout(server_t *server)
{
    ...
    for (int i = 0; i < MAX_CLIENTS; ++i) {
        ...
        if (min_t == -1 || food_timeout < min_t)
            min_t = food_timeout;
        if (client->queue->size > 0)
            min_t = calculate_next_event_cmd(client->queue, &now, min_t);
    }
    if (min_t == -1)
        return 1000;
    return (int)min_t;
